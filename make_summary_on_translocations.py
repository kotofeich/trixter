...
def overlap_common_entries(entries, type):
    ...
    while (entries_by_key_all):
        ...
        related = filter(lambda x: set(x) <= set(key_list) or
                         set(x) >= set(key_list), entries_by_key_all)
        for x in related:
                ...
                if type in line:
                    if chr_name2:
                        if not chr_name1:
                            ...
                            chr_name1, chr_name2 = sorted([chr_name1,
                                                           chr_name2])
                        summary.append(TranslocationEntry(
                            chr_name1, chr_name2, specie, related_blocks))
                        related_blocks = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dir', required=True,
                        help='file with rearrangements')
    parser.add_argument('--type', required=True,
                        help='translocation/transposition')
