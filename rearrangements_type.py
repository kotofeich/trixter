...
check if it's rearranged. sometimes we can find multiple possible previous
entries. in this case we need to know which one is previous in this case
...
def check_transpositions(c):
    ...
    # transpositions = sorted(transpositions, key = lambda x: x.start,
    # reverse=True)
...
the main chromosome is the one that corresponds to less translocations (by
length) generally translocations can be counted without information about
previous entries because they are already grouped into lists of unbroken
segments of blocks. however we report translocations in the same manner as
reversals and transpositions for uniformity and for being able to report the
...
def check_translocations(c):
    ...
    lengths = map(lambda y: sum(map(
        lambda x: math.fabs(int(x.get_end()) - int(x.get_start())), y)),
        c_seq_ids)
...
here we use the normalization of two genomes. i.e. we brought the genome of
specie1 to the form where all the entries have '+' strand, changing also the
strand of the corresponding entries in specie2 every '-' is called reversal
